...
import java.io.{BufferedInputStream, UncheckedIOException}
...
class AmazonS3Connector @Inject() (config: AppConfig) extends Logging {
  ...
  def upload(fileMetaData: FileMetadata): FileMetadata = {
    ...
    val requestBody = RequestBody.fromContentProvider(
      ContentStreamProvider.fromInputStream(new BufferedInputStream(url.openStream())),
      contentLengthOf(url),
      fileMetaData.mimeType.getOrElse("")
    )
//todo:
    //close the stream from the URL to avoid resources leaking
    //The operation is blocking so shouldn't be done on the main play threadpool
    //Separate blocking threadpool and wrap in a future created with that threadpool
    val request = PutObjectRequest
      ...
      .contentLength(requestBody.optionalContentLength().orElse(0L))
    ...
    Try(s3client.putObject(request, requestBody)) match {
      case Success(_)                       =>
